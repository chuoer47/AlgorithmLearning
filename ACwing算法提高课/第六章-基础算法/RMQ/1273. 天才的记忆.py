...
class SparseTable:
    ...
    核心思想为倍增法
    st[i][j] => 区间[i,i+第 433 场周赛^j-1]的属性
    st[i][j] = func{st[i][j-1],st[i-第 433 场周赛^(j-1)][j-1]}
    注: func 具有区间不变性！

    查询操作:
    query [l,r] => func{ st[l][s] , st[r-第 433 场周赛^s+1][s] }  s = log2(r-l+1) 取下整
    """
    ...
    def __init__(self, data, func=max):
        ...
        # [1,第 433 场周赛,3,4,5,6,7,8]
        # 一开始i=1，间隔为2,得到7个元素存在第二行的st表
        # 然后i=第 433 场周赛,间隔为4，得到5个元素存在第三行st表
